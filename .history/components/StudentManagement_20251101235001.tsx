...
export default function StudentManagement({ userData }: Props) {
  ...
  const fetchStudent = async (id: string, mode: 'view' | 'edit') => {
    ...
    try {
      ...
      // Prepare form state for edit or view reuse
      setForm({
        name: data.name,
        ...
        // keep subjects as returned, to render names and send back ids
  ...
  const saveStudent = async () => {
    ...
    try {
      ...
      // Ensure subjects payload is in ids list or StudentSubject ids if backend expects that
      // Assuming backend expects StudentSubject IDs list for updates or full object-safe PUT
      const payload: any = {
        ...form,
        // normalize subjects to StudentSubject ids if present
        subjects: Array.isArray(form.subjects)
          ? form.subjects.map(ss => ss._id)
          : [],
      };
      ...
      setForm({
        ...form,
        // re-sync with actual response
        subjects: updated.subjects || [],
      });

      // Update the list view if student is part of the tutor.students
      setStudents(prev =>
          ...
          <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
            ...
            <View style={styles.formRow}>
              ...
              <TextInput
                ...
                onChangeText={t =>
                  setForm(f => ({ ...f, status: t === 'inactive' ? 'inactive' : 'active' }))
                }
                editable={editable}

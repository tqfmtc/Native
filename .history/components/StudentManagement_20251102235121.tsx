...
export default function StudentManagement({ userData, onBack }: Props) {
  ...
  const saveStudent = async () => {
    ...
    try {
      ...
      setForm({ ...form, subjects: updated.subjects || [] });
  ...
  const SubjectPills = ({ subjects }: { subjects?: StudentSubject[] }) =>
    !subjects || subjects.length === 0 ? (
      <Text style={styles.muted}>No subjects</Text>
    ) : (
      ...
      <View style={styles.modalBody}>
        {studentLoading ? (
          ...
          <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
            ...
            <FormField
              label="Name"
              value={editable ? form.name : studentDetail.name}
              onChangeText={t => setForm(f => ({ ...f, name: t }))}
              editable={editable}
            />
            <FormField
              label="Father's Name"
              value={editable ? form.fatherName : studentDetail.fatherName}
              onChangeText={t => setForm(f => ({ ...f, fatherName: t }))}
              editable={editable}
            />
            <FormField
              label="Contact"
              value={editable ? form.contact : studentDetail.contact}
            ...
            <View style={styles.section}>
              ...
              <SubjectPills
                subjects={editable ? (form.subjects as StudentSubject[]) : studentDetail.subjects}
              />
            </View>

            <FormField
              label="Remarks"
              value={editable ? form.remarks : studentDetail.remarks}
  ...
  return (
    // ✅ Safe area excludes top inset — no forced padding at the top

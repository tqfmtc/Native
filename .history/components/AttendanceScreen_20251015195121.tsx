...
import { Announcement, getAnnouncements, getRecentAttendance, LoginResponse, markAttendance } from '../utils/api';
      ...
      <View style={styles.attendanceContainer}>
        <TouchableOpacity
          style={[
            ...
            loading && styles.attendanceButtonDisabled,
          ]}
          onPress={handleMarkAttendance}
          disabled={loading || attendanceMarked} // Removed Sunday condition from disabled
        >
          {loading ? (
            <ActivityIndicator size="large" color="#fff" />
          ) : (
            <Text style={styles.attendanceButtonText}>
              {attendanceMarked ? 'Attendance Marked Successfully' : 'Mark Attendance'}
            </Text>
          )}
        </TouchableOpacity>
          ...
          <View style={styles.distanceInfo}>
            <Text style={styles.distanceText}>
              Distance to center: {Math.round(calculateDistance(
                currentLocation,
                {
                  lat: userData.assignedCenter.coordinates[0], // latitude (first element)
                  lng: userData.assignedCenter.coordinates[1], // longitude (second element)
                }
              ))}m
            </Text>
            ...
            source={{
              html: `
                ...
                <body>
                  <div id="map"></div>
                  <script>
                    const centerLat = ${userData.assignedCenter.coordinates[0]}; // latitude (first element)
                    const centerLng = ${userData.assignedCenter.coordinates[1]}; // longitude (second element)
